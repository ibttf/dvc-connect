...
const Home = (props) => {
  const [dayOfTheWeek, setDayOfTheWeek] = useState("Any Day");
  const [hours, setHours] = useState("Any Time");
  const [isDayOfTheWeekDropdownVisible, setIsDayOfTheWeekDropdownVisible] =
    useState(false);
  const [isHoursDropdownVisible, setIsHoursDropdownVisible] = useState(false);
  const [isSubjectDropdownVisible, setIsSubjectDropdownVisible] =
    useState(false);
  const [selectedSubject, setSelectedSubject] = useState("Any Subject");

  function formatHour(hour) {
    if (hour === props.t("Any Time")) {
      return "Any Time";
    }
  ...
  return (
    <div
      className="lg:pb-24  lg:my-12 pb-12 my-6"
      style={{
        display: "flex",
    ...
    >
      ...
      <div className="md:w-108 w-11/12 mx-auto md:py-8 py-4">
        <div className="grid md:grid-cols-3 md:first-letter:gap-4 gap-2 grid-cols-1">
            ...
            <div className="relative w-full">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setIsHoursDropdownVisible(false);
                  setIsDayOfTheWeekDropdownVisible(
              ...
              >
                ...
                <svg
                  className="fill-current h-4 w-4 transition-transform transform-gpu"
          ...
          <div className="flex flex-col space-y-2 items-start">
            ...
            <div className="relative w-full">
              <button
                className=" w-full text-left text-md text-gray-900 py-2 px-3 bg-white border border-gray-300 rounded shadow-sm flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-gray-500"
                onClick={(e) => {
                  e.stopPropagation();
      ...
      <Link
        to={`/${dayOfTheWeek}/${hours}/${selectedSubject}`}
